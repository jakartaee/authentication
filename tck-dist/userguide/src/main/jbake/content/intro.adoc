...
The test suite is the collection of JUnit tests used with the Maven Surefire plugin to test a particular technology implementation. In this case, it is the
...
properties and run the {TechnologyShortName} tests, as described in
...
on the system hosting the Maven Surefire plugin run:
